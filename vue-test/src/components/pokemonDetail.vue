                ...
                <div class="stats">
                    ...
                    <div class="detail__wrap">
                        <div v-for="(stat, index) in pokemon.stats" :key="index" class="tag">
                            <div class="tag__label" :class="stat.stat.name">{{ formatName(stat.stat.name) }}</div>
...
<script setup>
import { ref, onMounted } from 'vue'
const pokemon = ref(JSON.parse(sessionStorage.getItem('selectpokemon')))
...
onMounted(() => {
    console.log(1111)
    console.log(sessionStorage.getItem('selectpokemon'));


    if (pokemon.value.species) {
        getPokemonDescription(pokemon.value.species.url)
    }
})
